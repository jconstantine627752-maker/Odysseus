...
class RiskAssessment {
    ...
    async assessToken(token, chain = 'x402') {
        try {
            ...
            logger_1.logger.error('Token risk assessment failed:', error);
            throw error;
        }
    }
    async assessProtocol(address, chain = 'x402') {
        try {
            ...
            logger_1.logger.error('Protocol risk assessment failed:', error);
            throw error;
        }
    }
    async getPortfolioExposure(timeframe) {
        try {
            const exposure = await this.calculatePortfolioExposure(timeframe);
            return exposure;
        }
        catch (error) {
            logger_1.logger.error('Failed to calculate portfolio exposure:', error);
            throw error;
        }
    }
    async getRiskAlerts(params) {
        try {
            ...
            const types = ['position', 'portfolio', 'market', 'system'];
    ...
    async updateRiskLimits(limits) {
        try {
            ...
            logger_1.logger.info('Risk limits updated successfully');
            return {
                success: true,
                limits,
    ...
    async getDrawdownAnalysis(timeframe) {
        try {
            return {
                timeframe,
    ...
    async getValueAtRisk(params) {
        try {
            ...
            logger_1.logger.error('Failed to calculate Value at Risk:', error);
            throw error;
        }
    }
    async performTokenAnalysis(token, chain) {
        ...
        const riskScore = factors.reduce((score, factor) => score + (factor.score * factor.weight), 0);
        let riskLevel;
        if (riskScore > 80)
            riskLevel = 'low';
    ...
    async performProtocolAnalysis(address, chain) {
        return {
            address,
            chain,
    ...
    async calculatePortfolioExposure(timeframe) {
        return {
