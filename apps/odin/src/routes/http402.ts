...
import { requirePayment, paymentService } from '../services/x402';
...
router.get('/premium-data', 
  requirePayment('0.10', 'Premium market data access', 'base'),
  (req, res) => {
    ...
    logger.info(`Premium data accessed via X402 payment`);
    res.json(marketData);
  }
);
...
router.post('/ai-analysis',
  requirePayment('0.25', 'AI market analysis service', 'base'),
  (req, res) => {
    ...
    logger.info(`AI analysis generated via X402 payment for ${symbol || 'ETH'}`);
    res.json(analysis);
  }
);
 ...
 */
router.get('/realtime-alerts/:symbol',
  requirePayment('0.05', 'Real-time price alert setup', 'base'),
  (req, res) => {
    ...
    logger.info(`Real-time alert set up via X402 payment: ${symbol} ${direction} ${threshold}`);
    res.json(alertSetup);
  }
);
 ...
 */
router.get('/mev-protection/:txHash',
  requirePayment('0.15', 'MEV protection analysis', 'base'),
  (req, res) => {
    ...
    logger.info(`MEV protection analysis completed via X402 payment for ${txHash}`);
 ...
 */
router.get('/cross-chain-rates',
  requirePayment('0.02', 'Cross-chain bridge rates', 'base'),
  (req, res) => {
    const bridgeRates = {
      timestamp: new Date().toISOString(),
      routes: [
    ...
    logger.info(`Cross-chain rates accessed via X402 payment`);
